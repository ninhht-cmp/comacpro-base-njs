...
// ---------------------------------------------------------------------------
// Zod emission — runtime validators, mirroring the type emitter from the SAME
// schema nodes so the two artifacts cannot drift from each other.
// ---------------------------------------------------------------------------

/** Render a schema node to a zod expression. */
function zodType(node: SchemaNode, where: string): string {
  const orNull = (t: string) => (node.nullable ? `${t}.nullable()` : t);

  if (node.$ref) {
    const match = SCHEMA_REF.exec(node.$ref);
    if (!match) fail(`unsupported $ref "${node.$ref}" at ${where}`);
    // Lazy so declaration order (and any future cycles) never matters.
    return orNull(`z.lazy(() => ${match[1]!}Schema)`);
  }
  if (typeof node['x-enum-name'] === 'string') {
    return orNull(`${node['x-enum-name']}Schema`);
  }
  if (node.anyOf) fail(`anyOf at ${where} is not supported yet`);
  if (node.allOf) {
    return orNull(
      node.allOf
        .map((p, i) => zodType(p, `${where}.allOf[${i}]`))
        .reduce((acc, part) => `${acc}.and(${part})`),
    );
  }
  if (node.oneOf) {
    const parts = node.oneOf.map((p, i) => zodType(p, `${where}.oneOf[${i}]`));
    return orNull(
  ...
  switch (node.type) {
    case 'string':
      return orNull('z.string()');
    case 'integer':
    case 'number':
      return orNull('z.number()');
    case 'boolean':
      return orNull('z.boolean()');
    case 'array':
      if (!node.items) fail(`array without items at ${where}`);
      return orNull(`z.array(${zodType(node.items, `${where}[]`)})`);
    case 'object':
    case undefined: {
      if (node.properties) return orNull(zodObject(node, where));
      if (typeof node.additionalProperties === 'object') {
        const value = zodType(node.additionalProperties, `${where}{}`);
        return orNull(`z.record(z.string(), ${value})`);
      }
      return orNull('z.record(z.string(), z.unknown())');
    }
    default:
      fail(`unsupported type "${node.type}" at ${where}`);
  }
...
function zodEnumExpr(values: (string | number)[]): string {
  if (values.every((v) => typeof v === 'string')) {
    return `z.enum([${values.map(literal).join(', ')}])`;
  }
  const literals = values.map((v) => `z.literal(${literal(v)})`);
...
function emitZodSchemas(
  ...
  const header = [
    ...
    ' */',
    "import { z } from 'zod';",
    `import type { ${[...enums.map((e) => e.name), ...names.filter((n) => !enums.some((e) => e.name === n))].sort().join(', ')} } from './model';`,
  ].join('\n');

  const enumBlocks = enums.map(
    (def) =>
      `export const ${def.name}Schema = ${zodEnumExpr(def.values)} satisfies z.ZodType<${def.name}>;`,
  );

  const schemaBlocks = names
    ...
    .map((name) => {
      ...
      if (Array.isArray(schema.enum)) {
        ...
        return `export const ${name}Schema = ${zodEnumExpr(values)} satisfies z.ZodType<${name}>;`;
      }
      return `export const ${name}Schema = ${zodType(schema, name)} satisfies z.ZodType<${name}>;`;
    });

  return `${[header, ...enumBlocks, ...schemaBlocks].join('\n\n')}\n`;
}
...
export interface GenerateResult {
  /** `model/index.ts` — the TypeScript types. */
  content: string;
  /** `schemas.ts` — the zod runtime validators. */
  schemas: string;
...
export function generateModels(
  ...
  return {
    content: `${blocks.join('\n\n')}\n`,
    schemas: emitZodSchemas(names, schemas, enums),
    stats: {
