...
4. **MSW** (`src/mocks`): node server (qua `instrumentation.ts`), bật bằng `NEXT_PUBLIC_API_MOCKING=enabled` — **server-side only** vì app RSC-first, mọi API call xuất phát từ server (browser worker đã gỡ). `handlers.ts` viết tay, envelope-accurate theo SaleNet, phủ vertical slice (signin → profile → notifications → referral).
