...
function Input({ className, type, ...props }: React.ComponentProps<'input'>) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        // h-10/px-3 instead of shadcn's dense h-8: every form in this product
        // is filled by sales staff on phones, so the comfortable touch-target
        // size IS the default. Add a size variant only when a genuinely dense
        // context (e.g. admin tables) shows up.
