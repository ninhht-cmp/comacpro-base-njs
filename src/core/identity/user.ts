import { UserRole, UserStatus } from '@/lib/api/generated/model';

/**
 * Canonical user-domain enums, shared by the session core and the users
 ...
 * exhaustive `Record<UserRole, …>` (labels, future authz) fails to compile —
 * the same drift guarantee the old numeric mappers provided. The domain names
 * themselves come from `openapi/codegen.json` (the generator deduplicates the
 * spec's inline enums into these single definitions).
 */
